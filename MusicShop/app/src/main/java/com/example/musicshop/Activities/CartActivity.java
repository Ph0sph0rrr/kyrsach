...
import com.example.musicshop.Adapters.CartAdapter;
    ...
    private void showOrderDialog() {
        ...
        builder.setPositiveButton("Оформить", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                String regexSTR = "^\\(?[0-9]{1,3}\\)? ?-?[0-9]{1,3} ?-?[0-9]{3,5} ?-?[0-9]{4}( ?-?[0-9]{3})?";
                String phoneNumber = etPhoneNumber.getText().toString().trim();
                if(etPhoneNumber.getText().toString().length()<10 || phoneNumber.length()>13 || phoneNumber.matches(regexSTR)==false){
                    Toast.makeText(CartActivity.this, "Не правильный номер", Toast.LENGTH_SHORT).show();
                }
                else {

                    if (!TextUtils.isEmpty(phoneNumber)) {
                        String orderId = generateOrderId();
                        Order order = new Order(orderId, phoneNumber, new ArrayList<>(cartItems));;
